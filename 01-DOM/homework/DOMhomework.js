...
span.innerHTML = 'Aplicación creada por: Ciro'
...
function buildToDo(todo, index) {
   ...
   // 3) Crear un elemento 'span' y asignarlo a una variable denominada 'toDoText'
   let toDoText = document.createElement('span')
   //4) Utilizando el objeto toDo pasado como argumento, setear el 'toDoText' innerHTML
   toDoText.innerHTML = todo.description
   //5) Asignarle como id del 'toDoText' el valor 'index' recibido como argumento
   toDoText.id = index
   //6)n función del atributo 'complete' del objeto ToDo recibido como argumento:
   if(todo.complete){
...
function buildToDos(toDos) {
  // Tu código acá:
  let toDoElements = toDos.map((todo,index)=>buildToDo(todo,index))
  return toDoElements
}
...
function displayToDos() {
  // Tu código acá:
  //  Paso 1: Seleccionr el elemento cuyo id es 'toDoContainer' y almacenarlo en una variable denominada 'toDoContainer'
...
function addToDo() {
  // Tu código acá:
  // Paso 1: Crear un nuevo ToDo usando la clase ToDo y pasándole el valor del input 'toDoInput' como parámetro
